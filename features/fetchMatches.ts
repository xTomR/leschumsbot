...
export default (client: Client,) => {
        ...
        const users = await usersSchema.find({}).exec()
            for(const eachUser of users){
                ...
                .exec()
                await usersSchema.findOneAndUpdate({puuid: eachUser.puuid,},{$addToSet: {matches: matchIds,}})
        ...
        const matchInfo = async () => {
            ...
            for(const eachMatch of totalmatches){
                const getMatchInfo = await galeforce.lol.match.match()
                .region(galeforce.region.riot.AMERICAS)
                .matchId(eachMatch)
                .exec()
                await matchSchema.findOneAndUpdate({_id: eachMatch},{queueId: getMatchInfo.info.queueId, participants: getMatchInfo.metadata.participants, lpMultiplier: 0,}, {upsert: true})                                
            }   
        }
        ...
        const lpIncMatches = async () => {
            ...
            for(const eachUser of users){
               await matchSchema.updateMany({ $and: [{'participants': eachUser.puuid}, {'done': false}]}, { $inc:{'lpmultiplier': 1}})            
            }
